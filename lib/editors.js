...
function observeTextEditors(editor) {
	...
	editors[file] = {
		...
		gutter: editor.addGutter({ name: "gdb-debug", priority: -100 })
	};
...
function openAndHighlight(stack) {
	...
	if (stack.addr === lastStackAddr) {
		return;
	}
	lastStackAddr = stack.addr;
	...
	atom.workspace.open(stack.file, { initialLine: line, searchAllPanes: true }).then(() => {
		...
		editor.decorateMarker(lineMarker, { type: "line", class: "gdb-debug-debug-line" });
...
function updateMarker(file, line, bp) {
	...
	el.className = "gdb-debug-breakpoint gdb-debug-breakpoint-state-" + bp.state;
	el.dataset.state = bp.state;
	el.title = bp.message || ""; // TODO: add texts for other breakpoint states
	const decoration = {
		class: "gdb-debug-gutter-breakpoint",
		item: el
	};
...
function onMarkerDidChange({ file, line, marker }, event) {
	...
	GDB.updateBreakpointLine(file, line, marker.getStartBufferPosition().row);
}
...
const debouncedStoreChange = debounce(() => {
	...
	openAndHighlight(state.gdb.stacktrace[state.gdb.selectedStacktrace]);
}, 50);
